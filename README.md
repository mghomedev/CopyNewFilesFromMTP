...
- **MTP performance**: MTP (Media Transfer Protocol) has some inherent per-file overhead compared to direct file system access. The script mitigates this by caching MTP folder enumerations (avoiding slow per-file `ParseName` lookups) and minimizing filesystem operations per copy. Actual transfer speed depends on USB connection speed and file sizes. Use `-Benchmark` to measure real throughput on your system.
